...
export const UPDATE_FIELD = "advisersUpdateField";
export const LIST_LOAD = "advisersListLoad";
export const SMILARS_LIST_LOAD = "advisersSmilarsListLoad";
export const START_FETCH = "advisersStartFetch";
export const LOAD_MORE = "advisersMoreFetch";
export const LOAD_ADVISER = "loadAdviser";
export const REGISTER_ADVISER = "registerAdviser";
export const RESET_REGISTER_ADVISER = "resetRegisterAdviser";
import copy from "copy-to-clipboard";
import {getContactDetail} from "../contacts";
import { toEnglishDigits } from "../../src/components/Kit/Utils";

export const advisersUpdateField = ({prop, value}) => ({
    type: UPDATE_FIELD,
    payload: {prop, value},
});
...
export const addTeacher = (
    ...
    try {
        let data = {
            'about': aboutMe,
            'cityId': city[1].id,
            'courses': [...selectedCourses.map((item, index) => (
                    {
                        'courseMs': item.id,
                        'sequence': index + 1
                    }
            ...
            'images': imgsUploaded.length > 0 ? [...imgsUploaded.map(item => (
                item.hashCode
            ))] : [],
            'videos': video != null ? [video.hashCode] : [],
            'introduction': teacherInfo,
            'sheba': toEnglishDigits(sheba),
            'tutoring': privateTeaching,
            'userPhone': phoneNumber,
            'firstName': name,
            'lastName': familyName,
            'nationalCode': toEnglishDigits(countryCode)
        }
        let url = process.env.REACT_APP_BASE_URL_NEW + process.env.REACT_APP_ADD_TEACHER;
