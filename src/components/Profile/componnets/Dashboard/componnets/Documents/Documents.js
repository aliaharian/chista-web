...
import {updateInfo, loadUser, deleteFile, showMessage, errorSnackbar} from "../../../../../../../redux/user";
...
function Documents(props) {
    ...
    function handleSelectFile(event) {
        ...
        reader.onload = function (upload) {
            ...
            setFiles(tempFiles);
            props.updateInfo({ deedFiles: tempFiles.filter(item => item.local === true).map((item) => ({ file: item.file })) }, (response) => { setfiles(response.deedFiles) });
        ...
        <>
            ...
            <Grid container spacing={1} direction={"column"} alignItems="flex-start" justify='space-between' style={{ margin: '8px 0', width: "100%" }} >
                <div className={classes.fileBoxTitle}>
                    <Typography className={classes.inputLabel}>مدارک و مجوزها ( حداکثر {props.countLimit} فایل  {props.fileSize} مگابایت)</Typography>
                </div>
                        ...
                        {/* {props.load ? (
                            ...
                            />
                                ...
                                <>
                                    ...
                                    {props.canDelete && <DeleteForeverIcon className={classes.removeIcon} key={'delete' + item.id} onClick={() => { removeItem(item.id) }} />}
                                </>
                            )} */}
                        <img src={item.local ? item.file : transform.getImage(item.file)} key={'img' + item.id} />
                        {props.canDelete && <DeleteForeverIcon className={classes.removeIcon} key={'delete' + item.id} onClick={() => { removeItem(item.id) }} />}
