...
const ActivityPacketItem = (props) => {
    ...
    useEffect(() => {
        if (props.datas && props.datas.result) {
            let tmpXAxis = []
            let tmpMembers = []
            let tmpMembersData = []
            ...
            props.datas.result.usages.map((data) => {
                ...
                // tmpMembersData.push(data.members)
            })
            // } 
            // else {
            //     props.datas.result.usages.map((data) => {
    ...
    const options = {
        ...
        plotOptions: {
            series: {
                threshold: props.verticalNumbers[0],
                cursor: 'pointer',
                point: {
                    events: {
                        click: function () {
                ...
                formatter: function () {
                    if (props.day) {
                        if (dateTime.dateTimeCustom(this.value).day === '۷' || dateTime.dateTimeCustom(this.value).day === '۱۴' || dateTime.dateTimeCustom(this.value).day === '۲۱' || dateTime.dateTimeCustom(this.value).day === '۲۸') {
                            return `<p style="${isMobile ? 'width:max-content' : 'width:100px'}">${dateTime.dateTimeCustom(this.value).day} ${dateTime.dateTimeCustom(this.value).month}</p>`


                            // '<p style="'+isMobile?"width:40px":+"width:100px"+'">' + dateTime.dateTimeCustom(this.value).day + ' ' + dateTime.dateTimeCustom(this.value).month + '</p>'
                        }
                    } else {
                        if (dateTime.dateTimeCustom(this.value).hour === '۶' || dateTime.dateTimeCustom(this.value).hour === '۱۲' || dateTime.dateTimeCustom(this.value).hour === '۱۸') {
                            ...
                            // '<p style="width:20px">' + dateTime.dateTimeCustom(this.value).hour + ':۰۰</p>'
                        }
                    }
                }
            },
        ...
        tooltip: {
            ...
            formatter: function () {
                ...
                <div style="padding:15px 14px;min-height:54px;width:178px;display:flex;align-items:center;justify-content:center;flex-direction:column;">
                    <div style="width:100%;margin-bottom:10px;">
                        ...
                        ${dateTime.dateTimeCustom(key.time).day + " " + dateTime.dateTimeCustom(key.time).month + " " + dateTime.dateTimeCustom(key.time).year}
                        </p>
                    </div>
            ...
            `:
                            ...
                            <div style="width:100%;margin-bottom:10px;">
                                ...
                                ${dateTime.dateTimeCustom(key.time).day + " " + dateTime.dateTimeCustom(key.time).month + " " + dateTime.dateTimeCustom(key.time).year}
                                - 
                                ${dateTime.dateTimeCustom(key.time).hour + ":۰۰"}
                                 </p>
                            </div>
                            <div style="width:100%;display:flex;align-items:center;justify-content:space-between;flex-direction:row-reverse;">
                                <div style="display:flex;align-items:center;justify-content:flex-start;flex-direction:row-reverse;">
    ...
    return (
        <Grid container className={classes.packetItemParent}>
            <div className={classes.packetItem}>
                <div className={classes.chart}>
                    ...
                    {!props.day && <div className={classes.backToMonth} onClick={() => {
                        props.handleBackToMonth(props.datas?.result.usages[0])
